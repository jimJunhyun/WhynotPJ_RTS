...
{ 
	...
	public override void Gen(Vector3 startPos, Vector3 endPos, bool affectHeight, int id)
	{
		...
		children[1].transform.position = new Vector3(transform.position.x, children[0].transform.position.y+ 1.66f, transform.position.z);
		...
		for (int i = Mathf.CeilToInt(-(wallAmt / 2)); i < Mathf.FloorToInt( wallAmt / 2); ++i)
		{
			...
			//g.transform.localPosition = new Vector3(i * ConstructBuild.WALLXSCALE, g.transform.position.y - transform.position.y,0);
		}
